namespace Battleships
{
    ...
    public class Engine
    {
        ...
        {
            int attackerIndex = Rand.Next(0, this._ships.Count);
            int defenderIndex = Rand.Next(0, this._ships.Count);

            while (defenderIndex == attackerIndex)
            {
                defenderIndex = Rand.Next(0, this._ships.Count);
            }

            Ship attacker = this._ships[attackerIndex];
            Ship defender = this._ships[defenderIndex];
            ...
            var tmp = attacker as Battleship;
            return tmp.Attack(defender);

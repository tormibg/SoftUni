...
namespace Menu
{
    class Menu
    {
        ...
        private static void VoteForPizza()
        {
            ...
            string vote = RetrieveParameters["pizzaVote"];
            using (var pizzaMoreContext = new PizzaMoreContext())
            {
                Pizza pizza = pizzaMoreContext.Pizzas.FirstOrDefault(p => p.Id == pizzaId);
                if (vote == "up")
                {
                    pizza.UpVotes++;
                }
        ...
        {
            Header.Print();
            WebUtil.PrintFileContent(Constants.MenuTopHtml);
            GenerateNavbar();
            GenerateAllSuggestions();
            WebUtil.PrintFileContent(Constants.MenuBottomHtml);
        }

        private static void GenerateAllSuggestions()
        {
            var pizzaContext = new PizzaMoreContext();
        ...
        {
            //Console.WriteLine(Constants.MenuNavBarHtml);

namespace EnvironmentSystem.Models.Objects
{
    ...
    public class Snowflake : MovingObject
    {
        ...
        public override IEnumerable<EnvironmentObject> ProduceObjects()
        {
            if (!this.Exists)
            {
                var newObj = new Snow(this.Bounds.TopLeft.X, this.Bounds.TopLeft.Y-1, this.Bounds.Width,
                    this.Bounds.Height);
                List<EnvironmentObject> newObjects = new List<EnvironmentObject> {newObj};
                return newObjects;
            }
